...
// Msg Object
const revAuthCheckMsg = require('./revAuthCheckMsg')

async function mwAuthCheck (req, res) {
    ...
    if ( req.user ) {
        ...
        if ( rs.length > 0 ) {
            res.status(200).json(revAuthCheckMsg.allow)
          } else {
            res.status(452).json(revAuthCheckMsg.deny)
          }
    } else {
