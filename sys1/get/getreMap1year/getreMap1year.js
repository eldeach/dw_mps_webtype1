...
async function getreMap1year (app) {
    app.get('/getremap1year', async function ( req, res ) {
        let rsSetting = await sendQry(`SELECT periodic_mapping_1y, periodic_mapping_1y_offset_day FROM tb_period_setting WHERE no = (SELECT max(no) AS no FROM tb_period_setting)`)

        let period = rsSetting[0].periodic_mapping_1y
        let offset_day = rsSetting[0].periodic_mapping_1y_offset_day
        ...
        FROM (
            ...
            FROM
                ...
                FROM
                    ...
                    LEFT OUTER JOIN (
                        SELECT mc_periodic_1y_mt_id, MAX(doc_approval_date) AS MAX_approval_date FROM tb_mc_periodic_1y_mt GROUP BY mc_periodic_1y_mt_id
                    ) AS PR_Q_MAX_TBL
                    ON THIS_TBL.mc_periodic_1y_mt_id = PR_Q_MAX_TBL.mc_periodic_1y_mt_id
                WHERE
                    THIS_TBL.approval_status = 'APPROVED'
                    AND THIS_TBL.periodic_mng_1y_mt = 1
                ) AS INTER_TBL
        ) AS FINAL_TBL
        `.replace(/\n/g, "")
        )
