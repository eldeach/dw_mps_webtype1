...
async function getrequal5year (app) {
    app.get('/getprreview', async function ( req, res ) {
        let rsSetting = await sendQry(`SELECT periodic_qual_review, periodic_qual_review_offset_day FROM tb_period_setting WHERE no = (SELECT max(no) AS no FROM tb_period_setting)`)
        
        let period = rsSetting[0].periodic_qual_review
        let offset_day = rsSetting[0].periodic_qual_review_offset_day
        ...
        FROM (
            ...
            FROM
                ...
                FROM
                    ...
                    LEFT OUTER JOIN (
                        SELECT mc_periodic_review_id, MAX(doc_approval_date) AS MAX_approval_date FROM tb_mc_periodic_review GROUP BY mc_periodic_review_id
                    ) AS PR_Q_MAX_TBL
                    ON THIS_TBL.mc_periodic_review_id = PR_Q_MAX_TBL.mc_periodic_review_id
                WHERE
                    THIS_TBL.approval_status = 'APPROVED'
                    AND THIS_TBL.periodic_mng_review = 1
                ) AS INTER_TBL
        ) AS FINAL_TBL
        `.replace(/\n/g, "")
        )
